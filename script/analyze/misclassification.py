...
def main(cfg):
  ...
  for fold in range(cfg.data.num_folds):
    ...
    # 모델 준비
    model_path = os.path.join(ROOT_DIR, 'artifacts', model_name, f'{model_name}_fold{fold}.pt')
    ...
    with torch.no_grad():
      for batch in tqdm(analyze_loader):
        ...
        pred_list[f"{fold}"]["img"].extend(img_base64)
    recoder = []
  ...
  df.to_csv(os.path.join(ROOT_DIR, "data", "misclassification.csv"), index=False)
